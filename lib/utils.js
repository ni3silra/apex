...
export function getDeadlineStatus(deadline) {
  ...
  if (hoursLeft <= 4) return { label: `${hoursLeft}h left`, color: 'var(--danger)', urgency: 90 };
  if (isToday(date)) return { label: 'Due today', color: 'var(--warning)', urgency: 80 };
  if (isTomorrow(date)) return { label: 'Due tomorrow', color: 'var(--warning)', urgency: 60 };
